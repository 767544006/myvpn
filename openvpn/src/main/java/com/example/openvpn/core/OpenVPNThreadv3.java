...
import static com.example.openvpn.VpnProfile.AUTH_RETRY_NOINTERACT;
...
public class OpenVPNThreadv3 extends ClientAPI_OpenVPNClient implements Runnable, OpenVPNManagement {
    ...
    private boolean setConfig(String vpnconfig) {
        ...
        config.setEachTimeout(15);
        config.setConnTimeout(30);
    ...
    public void log(ClientAPI_LogInfo arg0) {
        ...
        if (logmsg.contains("UNUSED OPTIONS")){
            VPN.INSTANCE.getConnectionInfo().clear();
            VPN.INSTANCE.setIsbegin(false);
        }
        if (logmsg.contains("Contacting")&&!logmsg.contains("to")){
            if (VPN.INSTANCE.getIsbegin()){
                useTime=System.currentTimeMillis()-timeStart;
                VPN.INSTANCE.getConnectionInfo().add(new ConnectState(currentIp,currentPort,1,useTime,type,timeStart));
            }
            timeStart=System.currentTimeMillis();
            String ipInfo =logmsg.split(" ")[1];
            currentIp=ipInfo.split(":")[0];
            currentPort=Integer.parseInt(ipInfo.split(":")[1]);
            if(logmsg.split(" ")[3].equals("UDP")) type=1; else type=2;
            VPN.INSTANCE.setIsbegin(true);
        }
        if (logmsg.contains("Connected via tun")){
            useTime=System.currentTimeMillis()-timeStart;
            VPN.INSTANCE.getConnectionInfo().add(new ConnectState(currentIp,currentPort,0,useTime,type,timeStart));
        }
